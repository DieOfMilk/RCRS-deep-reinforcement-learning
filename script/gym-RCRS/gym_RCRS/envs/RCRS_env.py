...
class RCRSEnv(gym.Env):
    ...
    def __init__(self, portNo, grpcNo, maxTimeStamp, buildingNo,mapName, verbose=False): # AreaList, agentList, algorithm,
        ...
        self.observation_space = spaces.Box(low=np.array([0]*(buildingNo*3+len(self.agentList))+[-inf,-inf]*len(self.agentList)+[0]*len(self.agentList)).astype(int),high=np.array([inf]*(buildingNo*3)+[self.maxWater]*len(self.agentList)+[inf,inf]*len(self.agentList)+[1]*len(self.agentList)).astype(int),dtype= np.int) ## id, is on fire
        self.obs= {}
