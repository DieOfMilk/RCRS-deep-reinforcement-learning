    ...
    def mylearn(self, save_path, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2", reset_num_timesteps=False):
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                temp_data_path = os.path.join(save_path,'temp')
...
if __name__=='__main__':
    ...
    # 
    # ipdb.set_trace()
    model = myPPO2(MlpPolicy, env, verbose=0, learning_rate=learning_rate)
    save_path = os.path.join('./log',mapName)
    if not os.path.exists(save_path):
        os.makedirs(save_path)
        
    model.mylearn(total_timesteps = 100000, save_path = save_path)
    model.save(log_dir+'{}'.format(learning_rate))
    temp.killprocess()
    env.close()
