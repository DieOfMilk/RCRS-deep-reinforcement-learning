...
public class ActionExtClear extends ExtAction
{
    ...
    public ExtAction myClear()
    {
        this.result = null;
        if (this.target == null)
        {
            System.out.println("no target define");
        ...
        EntityID agentPosition = policeForce.getPosition();
        List<EntityID> path = this.pathPlanning.getResult(agentPosition, this.target);
        StandardEntity targetEntity = null;
        ...
        for(EntityID elements: path) {
            targetEntity = this.worldInfo.getEntity(elements);
            if (targetEntity == null || !(targetEntity instanceof Area) || (targetEntity instanceof Building)) {
                continue;
            }
        ...
        if (clearBlockade != null)
        {
            ...
            this.result = new ActionClear(clearX, clearY, clearBlockade);
        }
        else {
            System.out.println("no clearblockade define");
            this.result = new ActionRest();
        }
        return this;
    }
    ...
    private Action myClearAction(PoliceForce police, StandardEntity targetEntity)
    {
        ...
        for (Blockade blockade : blockades)
        {
            // System.out.println(blockade.getID().getValue());
        ...
        if (clearBlockade != null)
        {
            ...
            //     System.out.println("Clear distance is " + Integer.toString(this.clearDistance));
            //     System.out.println("minimumDistance is "+ Integer.toString(minDistance));
    ...
    private Vector2D scaleMax(Vector2D vector)
    {
        return vector.normalised().scale(500);
    }
